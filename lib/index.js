...
var AbstractScheduleService = class extends Service {
  ...
  uploaderIntervalId;
  abortController;
  lastRoundStats = { totalProcessed: 0, maxVideoProcess: 0, isRunning: false, currentProcessed: 0, currentTotal: 0, currentSuccess: 0, currentFailure: 0 };
    ...
    try {
      ...
      for (const video of videosToUpdate) {
        ...
        const p = this.processSingleVideo(video).then((result) => {
          if (result === "changed") {
            totalSuccess++;
            this.lastRoundStats.currentSuccess++;
            ...
            totalFailure++;
            this.lastRoundStats.currentFailure++;
          }
  ...
  async processSingleVideo(video) {
    ...
    try {
      ...
      if (dataHasChanged && latestStat) {
        ...
        if (milestonesCrossed.length > 0) {
          ...
          for (const milestone of milestonesCrossed) {
            ...
            this.ctx.emit("lfvs/milestone-reached", video, milestone, latestStat, fullStat);
          }
        }
      }
      let newInterval;
      ...
      this.ctx.emit("lfvs/video-updated", this.platform, video, "success", costMs, latestStat, fullNewStat);
